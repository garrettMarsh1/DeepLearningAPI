...
def store_translation(t):
    model = TranslationModel(text=t.text, starting_language=t.starting_language, target_language=t.target_language)
    model.save()
    return model.id
...
def run_translation(t_id: int):
    ...
    prefix = f"translate {model.starting_language} to {model.target_language}: {model.text}"
